...
from .models import ClienteDestacado, MiembroEquipoPublicado, PreguntaFrecuente, ServicioPublicado
...
class ServicioPublicadoAdmin(admin.ModelAdmin):
    list_display = (
        'titulo_publico', 'servicio', 'categoria', 'subcategoria',
        'destacado', 'activo', 'orden', aviso_meta_description, 'actualizado',
    )
    list_editable = ('destacado', 'activo', 'orden')
    list_filter = ('activo', 'destacado', 'categoria', 'subcategoria')
    ...
    fieldsets = (
        ('Vínculo con el LIMS', {'fields': ('servicio', 'categoria', 'subcategoria')}),
        ('Contenido', {
