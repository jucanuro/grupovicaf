...
def buscar_clientes_api(request):
    try:
        ...
        if len(query) > 50:
            logger.info(
                f"Consulta larga en buscar_clientes_api por usuario {request.user.username}: {query[:50]}..."
            )
...
def crear_editar_cliente(request, pk=None):
    if pk:
        if not trabajador_tiene_permiso(request.user, 'clientes.editar'):
            return HttpResponseForbidden("No tienes permiso para editar clientes.")
    else:
        if not trabajador_tiene_permiso(request.user, 'clientes.crear'):
...
def confirmar_eliminar_cliente(request, pk):
    ...
    if request.method == 'POST':
        try:
            ...
            messages.success(
                request,
                f"Cliente '{nombre}' eliminado permanentemente."
            )
            ...
            messages.error(
                request,
                f"No se pudo eliminar el cliente '{cliente.razon_social}'. Tiene datos relacionados (proyectos/cotizaciones) que impiden su eliminación."
            )
            ...
            messages.error(
                request,
                f"Error al intentar eliminar el cliente: {e}"
            )
    ...
    return render(
        request,
        'clientes/clientes_confirm_delete.html',
        {'cliente': cliente}
    )
...
def crear_cliente_ajax(request):
    ...
    try:
        ...
        logger.info(
            f"Cliente creado exitosamente: {razon_social} (RUC: {ruc}) por usuario {request.user.username}"
        )
