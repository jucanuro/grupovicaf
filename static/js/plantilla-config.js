document.addEventListener('DOMContentLoaded', () => {
    ...
    const CURRENT_ENTITY_ID = config.cotizacionId || config.plantillaId || null;
    const CONDICIONES_ENDPOINT = config.condicionesJsonUrl || (config.cotizacionId ? `/servicios/cotizaciones/${config.cotizacionId}/condiciones/json/` : null);
    const CONDICIONES_GUARDAR_ENDPOINT = config.condicionesGuardarUrl || null;
    const CONDICIONES_RESUMEN_ENDPOINT = config.condicionesResumenUrl || null;
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3 mb-3">
                                ...
                                <div class="flex flex-wrap gap-2 shrink-0">
                                    ${item.es_obligatorio ? `<span class="px-2.5 py-1 rounded-full bg-amber-100 text-amber-700 text-[10px] font-black uppercase">Obligatorio</span>` : ''}
                                    ${item.fue_editado ? `<span class="px-2.5 py-1 rounded-full bg-blue-100 text-blue-700 text-[10px] font-black uppercase">Editado</span>` : ''}
                                    ${item.editable_en_cotizacion ? `<span class="px-2.5 py-1 rounded-full bg-emerald-100 text-emerald-700 text-[10px] font-black uppercase">Editable</span>` : `<span class="px-2.5 py-1 rounded-full bg-slate-100 text-slate-600 text-[10px] font-black uppercase">Bloqueado</span>`}
                                </div>
                            </div>
    ...
    window.aplicarCondicionesAlFormulario = async () => {
        ...
        window.CondicionesCotizacionState.secciones = deepClone(
            window.CondicionesCotizacionState.draftSecciones || []
        );

        window.CondicionesCotizacionState.configurado = hasAnySelection(
            window.CondicionesCotizacionState.secciones
        );
        ...
        if (CURRENT_ENTITY_ID && CONDICIONES_GUARDAR_ENDPOINT) {
            try {
                await fetch(CONDICIONES_GUARDAR_ENDPOINT, {
                    method: 'POST',
                    headers: {
                    ...
                    body: JSON.stringify({
                        secciones: window.CondicionesCotizacionState.secciones
                    })
                });
            } catch (error) {
                console.error('No se pudieron guardar las condiciones por endpoint:', error);
            }
        }
